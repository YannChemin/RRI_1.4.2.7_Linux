...
! Allocate arrays in the GPU RAM
!$omp target enter data map(alloc: qg_idx, gammag_idx, fg_idx)
! Send values of arrays to their clones in the GPU RAM
!$omp target update to(qg_idx, gammag_idx, fg_idx)
! Ask GPU to flatten the loop and process in one go
...
enddo
! Get the array from the GPU RAM
!$omp target update from(fg_idx)
! Delete the arrays from the GPU RAM
!$omp target exit data map(delete: qg_idx, gammag_idx, fg_idx)

! make that a verification of the GPU process
do k = 1, slo_count
 write(*,*) fg_idx(k)
 write(*,*) gammag_idx(k)
 if (gammag_idx(k) .gt. 0.d0) write(*,*) ((qg_idx(1,k) + qg_idx(2,k) + qg_idx(3,k) + qg_idx(4,k)) / gammag_idx(k))
enddo
...
!$omp parallel do 
!private(kk,zb_p,hg_p,gammag_p,kg0_p,ksg_p,fpg_p,l,distance,len,zb_n,hg_n,gammag_n,kg0_n,ksg_n,fpg_n,dh,dif_p,dif_n,qg)
do k = 1, slo_count
